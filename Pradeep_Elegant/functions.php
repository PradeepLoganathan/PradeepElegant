<?php
define('THEME_NAME',"Pradeep Elegant");
...
function art_get_metadata_icons($icons = '', $class=''){
	...
	for($i = 0; $i < count($icons); $i++){
		...
		switch($icon){
			case 'date':
				$result[] = art_get_meta_icon('postdateicon', 18, 18) . sprintf( __('<span class="%1$s">Published</span> %2$s', THEME_NS),
								'date',
			...
			break;
			case 'author':
				$result[] = art_get_meta_icon('postauthoricon', 18, 18) . sprintf(__('<span class="%1$s">By</span> %2$s', THEME_NS),
								'author',
			...
			case 'edit':
				...
				$result[] = art_get_meta_icon('postediticon', 20, 20) . ob_get_clean();
			break;
		}
	}
